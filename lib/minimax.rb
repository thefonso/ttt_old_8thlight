module Algorithm
  class Minimax
   ...
   #
   # Intermediate Game States are ranked according to game turn and the available moves on board.
   ...
   # If it’s O’s turn, set the rank to that of the minimum move available. In other words, If a move will result in a loss, X should avoid it.
    ...
    # all below will be refactored away...just here for quick looking.
    
    # code from gamestate.rb #################################
    ...
    def ai_third_move(board)
        block_human_win_only(board) unless !attempt_win(board).nil? #<----------min unless !max
      return @move
    end # Intermediate Game State 
  
    def ai_fourth_move(board)
      if board.grid[:b2] == "O"
        block_human_win_and_random_move(board) unless !attempt_win(board).nil? #<----------min unless !max
      elsif board.grid[:b2] == "X"
        block_human_win_and_random_move(board)     #<----------min
      end
      return @move
    end # Intermediate Game State
